...
自动修复系统由多个组件协同工作：Coordinator 统筹 Detector/Strategist/Executor，Fixer 提供增强修复能力，Notification 提供通知能力，KubernetesService 负责与集群交互。
...
- 端点: `POST /api/v1/multi-agent/repairs/create`
- 请求体:
...
- 端点: `POST /api/v1/multi-agent/repairs/create-all`
- 请求体:
...
- 端点: `POST /api/v1/multi-agent/analysis/create`
- 请求体:
...
# 触发修复（新版多Agent接口）
curl -X POST http://localhost:8080/api/v1/multi-agent/repairs/create \
...
```bash
# 执行集群诊断（新版多Agent接口）
curl -X POST http://localhost:8080/api/v1/multi-agent/analysis/create \
  -H "Content-Type: application/json" \
  -d '{
    "cluster_name": "default"
  }'
...
项目包含一个自动化测试脚本`tests/test-autofix.py`，用于验证自动修复功能。该脚本将逐步迁移到新版多Agent端点。
