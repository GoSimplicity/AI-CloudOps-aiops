...
def cleanup_test_environment():
    ...
    try:
        ...
        for yaml_file in yaml_files:
            ...
            result = subprocess.run(
                ["kubectl", "delete", "-f", str(file_path)],
                capture_output=True,
                text=True,
                check=False,
            )
...
def verify_pod_status(deployment_name, namespace="default"):
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            return {"success": False, "message": f"获取Pod信息失败: {result.stderr}"}
        ...
        if not pods:
            ...
            return {"success": False, "message": f"未找到部署 {deployment_name} 的Pod"}
        ...
        for pod in pods:
            ...
            restart_count = (
                sum(cs.get("restartCount", 0) for cs in container_statuses)
                if container_statuses
                else 0
            )
            ...
            logger.info(
                f"Pod {name}: 状态={phase}, {ready_status}, 重启次数={restart_count}"
            )
        ...
        return {"success": False, "message": f"验证Pod状态失败: {str(e)}"}
