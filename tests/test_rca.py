...
def make_request(
    method, url, json_data=None, max_retries=MAX_RETRIES, timeout=REQUEST_TIMEOUT
):
    """发送请求，包含重试逻辑"""
...
def setup_test_environment():
    ...
    try:
        ...
        result = subprocess.run(
            ["kubectl", "get", "nodes"], capture_output=True, text=True, check=False
        )
        if result.returncode != 0:
        ...
        subprocess.run(
            ["kubectl", "create", "namespace", "rca-test"],
            capture_output=True,
            text=True,
            check=False,
        )
...
def cleanup_test_environment():
    ...
    try:
        ...
        result = subprocess.run(
            ["kubectl", "delete", "namespace", "rca-test", "--ignore-not-found"],
            capture_output=True,
            text=True,
            check=False,
        )
...
def test_correlation_analysis():
    ...
    success = (
        response.status_code == 200
        and "data" in result
        and "correlations" in result["data"]
    )
...
def test_root_cause_analysis():
    ...
    success = (
        ...
        and "root_cause_candidates" in result["data"]
    )
...
def test_rca_with_specific_workload():
    ...
    success = response.status_code == 200 and "data" in result
...
def test_rca_performance():
    ...
    with ThreadPoolExecutor(max_workers=concurrent_requests) as executor:
        futures = [
            executor.submit(single_rca_request, i) for i in range(concurrent_requests)
        ]
        for future in futures:
            try:
                ...
                logger.info(
                    f"线程 {result['thread_id']} 完成: 成功={result['success']}, 耗时={result['duration']:.2f}秒"
                )
            except Exception as e:
    ...
    average_duration = sum(
        r.get("duration", 0) for r in results if r.get("duration")
    ) / len(results)
    ...
    logger.info(
        f"并发测试完成: 成功率={performance_summary['success_rate']:.1f}%, 平均耗时={average_duration:.2f}秒"
    )
...
def verify_test_environment():
    ...
    try:
        ...
        return {"success": True, "pod_count": len(pods), "pod_status": pod_status}
