...
class TestPredictionServiceDetailed:
    ...
    def test_get_workload_metrics_success(self, mock_prometheus, prediction_service, sample_metrics_data):
        """测试成功获取工作负载指标"""
    ...
    def test_get_workload_metrics_error(self, mock_prometheus, prediction_service):
        """测试获取指标时的错误处理"""
        mock_prometheus.return_value.query_range.side_effect = Exception("Prometheus连接失败")
        prediction_service.prometheus_service = mock_prometheus.return_value
    ...
    def test_calculate_time_factor_weekend(self, prediction_service):
        """测试周末时间因子计算"""
        # 创建一个周六的时间
        saturday = datetime(2024, 1, 6, 10, 0, 0, tzinfo=timezone.utc)  # 2024年1月6日是周六
        ...
        with patch.object(prediction_service, 'prometheus_service') as mock_prometheus:
            mock_prometheus.query_range.return_value = {
                ...
                "data": {
                    "result": [
                        {
                            "values": [[i * 60 + 1640995200, str(50 + i)] for i in range(30)]
                        }
                    ]
                }
            }
    ...
    def test_predict_with_custom_timestamp(self, prediction_service):
        """测试使用自定义时间戳的预测"""
        custom_time = datetime(2024, 6, 15, 14, 30, 0, tzinfo=timezone.utc)  # 工作日下午
...
class TestPredictionIntegration:
    ...
    def test_end_to_end_prediction_flow(self):
        """测试端到端预测流程"""
        with patch('app.core.prediction.predictor.PrometheusService') as mock_prometheus:
            # 设置模拟数据
            mock_prometheus.return_value.query_range.return_value = {
                "status": "success",
                "data": {
                    "result": [
                        {
                            "values": [[i * 60 + 1640995200, str(50 + i * 2)] for i in range(60)]
                        }
                    ]
                }
            }
...
if __name__ == "__main__":
    pytest.main([__file__, "-v", "--cov=app.core.prediction", "--cov-report=term-missing"])