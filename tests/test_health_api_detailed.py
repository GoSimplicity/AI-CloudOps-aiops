...
License: Apache 2.0
Description: 测试：test_health_api_detailed
"""
...
class TestDetailedHealthChecks:
    ...
    def test_detailed_health_all_services_healthy(
        self, mock_prometheus, mock_k8s, client
    ):
        """测试所有服务健康的详细检查"""
        # 模拟所有服务健康
    ...
    @patch("app.api.routes.health.KubernetesService")
    @patch("app.api.routes.health.PrometheusService")
    def test_detailed_health_some_services_unhealthy(
        self, mock_prometheus, mock_k8s, client
    ):
        """测试部分服务不健康的详细检查"""
        # 模拟部分服务不健康
    ...
    @patch("app.api.routes.health.KubernetesService")
    @patch("app.api.routes.health.PrometheusService")
    def test_detailed_health_all_services_unhealthy(
        self, mock_prometheus, mock_k8s, client
    ):
        """测试所有服务不健康的详细检查"""
        # 模拟所有服务不健康
...
class TestHealthCheckIntegration:
    ...
    @patch("psutil.cpu_percent")
    @patch("psutil.virtual_memory")
    def test_full_health_check_integration(
        self, mock_memory, mock_cpu, mock_prometheus, mock_k8s, client
    ):
        """测试完整的健康检查集成"""
        # 设置所有模拟服务为健康状态
        mock_k8s.return_value.check_connectivity.return_value = True
    ...
    def test_health_check_cascading_failures(self, client):
        """测试级联故障情况下的健康检查"""
        with (
            patch("app.api.routes.health.KubernetesService") as mock_k8s,
...
if __name__ == "__main__":
    pytest.main(
        [__file__, "-v", "--cov=app.api.routes.health", "--cov-report=term-missing"]
    )
