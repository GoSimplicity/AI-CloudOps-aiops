...
class TestAPIPerformance:
    ...
    def test_high_load_prediction_requests(self, client):
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = [executor.submit(make_prediction_request, i) for i in range(high_load_requests)]
            results = [future.result() for future in as_completed(futures)]
...
class TestPredictionServicePerformance:
    ...
    def test_concurrent_predictions_performance(self, prediction_service):
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = [executor.submit(make_prediction, i) for i in range(concurrent_count)]
            results = [future.result() for future in as_completed(futures)]
        end_time = time.time()
    ...
    def test_prediction_under_stress(self, prediction_service):
        ...
        start_time = time.time()
        for _ in range(5):
            thread = threading.Thread(target=lambda: results.append(stress_prediction()))
...
class TestScalabilityBenchmarks:
    ...
    def test_prediction_scalability_with_load(self, client, request_load):
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [executor.submit(make_prediction_request, i) for i in range(request_load)]
