...
License: Apache 2.0
Description: 多Agent 模块（coordinator）
"""
        ...
        try:
            ...
            for strategy_item in strategy.get("strategies", []):
                ...
                if not strategy_item.get("auto_fix", False):
                    continue
                # 命名空间白/黑名单检查
                if config.remediation.namespace_whitelist and (
                    namespace not in set(config.remediation.namespace_whitelist)
                ):
                    logger.info(f"跳过执行（不在白名单）: {deployment}/{namespace}")
                    continue
                if config.remediation.namespace_blacklist and (
                    namespace in set(config.remediation.namespace_blacklist)
                ):
                ...
                if config.remediation.safe_mode:
                    logger.info(
                        f"安全模式启用：仅告警不执行策略 {strategy_item.get('id')}"
                    )
            ...
            verification = await self.verifier.verify_deployment_health(
                deployment, namespace, wait_seconds=verify_wait
            )

            # 若验证失败，尝试回滚
            if (
                verification.get("status") == "failed"
                and config.remediation.allow_rollback
            ):
                logger.warning("验证失败，执行回滚...")
                await self.rollback.rollback_deployment(
                    deployment, namespace, reason="verification_failed"
                )
            ...
            self.metrics["successful_workflows"] += (
                1 if final_report.get("success") else 0
            )
            if verification.get("status") == "failed":
                self.metrics["rolled_back"] += (
                    1 if config.remediation.allow_rollback else 0
                )
...
class Coordinator:
    ...
    async def fix_deployment(self, namespace: str, deployment: str, issues: List[str]):
        return await self.agent.run_full_workflow(
