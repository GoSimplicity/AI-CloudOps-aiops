...
License: Apache 2.0
Description: 多Agent 模块（executor）
"""
...
class K8sExecutorAgent:
            ...
            for i, step in enumerate(steps):
                ...
                if not step_result["success"]:
                    result["errors"].append(
                        f"步骤 {i + 1} 失败: {step_result.get('error', '')}"
                    )
                    break
    ...
    async def _pre_check(self, strategy: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not await self._check_permissions(
                target.get("resource_type", "deployment"), namespace
            ):
                logger.error("权限检查失败")
                return False
    ...
    async def _execute_step(
        self, step: Dict[str, Any], target: Dict[str, Any]
    ) -> Dict[str, Any]:
        """执行单个步骤"""
        step_type = step.get("type", "")
        step_result = {"type": step_type, "success": False, "message": "", "error": ""}
            ...
            if target.get("resource_type") == "deployment":
                ...
                if deployment:
                    ...
                    if action == "collect_pod_events":
                        ...
                        return {
                            "success": True,
                            "message": f"已收集事件 {len(events)} 条",
                        }
                    if action == "collect_pod_logs":
                        tail = int(step.get("tail_lines", 200))
                        logs = await self.k8s_service.get_recent_pod_logs(
                            namespace=namespace,
                    ...
                    if action == "validate_image_pull":
                        ...
                        for pod in pods or []:
                            ...
                            spec_containers = (pod.get("spec", {}) or {}).get(
                                "containers"
                            ) or []
                            image_of = {
                        ...
                        suggestions = []
                        registries = sorted(
                            {a.get("registry") for a in affected if a.get("registry")}
                        )
                        if registries:
                            suggestions.append(
                                f"检查镜像仓库连通性与DNS解析：{', '.join(registries)}（网络出口/防火墙/代理）"
                            )
                        suggestions.append(
                            "为私有仓库配置 imagePullSecrets，并在 ServiceAccount/Pod 上引用"
                        )
                        # 汇总repo:tag
                        repos = []
                        for a in affected:
                        ...
                        if repos:
                            suggestions.append(
                                "确认以下镜像与标签已存在于仓库："
                                + ", ".join(sorted(set(repos))[:5])
                            )
                        suggestions.append(
                            "考虑使用镜像 digest（@sha256:...）固定版本，提升可重复性"
                        )
                        suggestions.append(
                            "根据发布策略设置 imagePullPolicy：latest/频繁更新用 Always；稳定版本用 IfNotPresent"
                        )
                        suggestions.append(
                            "确认节点/命名空间到镜像仓库的网络出口（NetworkPolicy/安全组/NAT）"
                        )
                        suggestions.append(
                            "如为自建不安全仓库，配置运行时 insecure registry 或启用 TLS"
                        )
                        suggestions.append(
                            "可重启 Deployment 以重试镜像拉取（kubectl rollout restart）"
                        )
                        return {
                            "success": True,
                    ...
                    if action == "validate_mount":
                        ...
                        for pod in pods:
                            ...
                            for cs in status.get("container_statuses", []) or []:
                                ...
                                if any(
                                    k in msg for k in ("mount", "mountvolume", "volume")
                                ):
                                    found = True
                                    break
                            if found:
    ...
    async def _execute_modify_step(
        self, step: Dict[str, Any], target: Dict[str, Any]
    ) -> Dict[str, Any]:
        """执行修改步骤"""
        try:
            ...
            if target.get("resource_type") == "deployment":
                ...
                if config.remediation.dry_run:
                    dry_ok = await self.k8s_service.patch_deployment(
                        ...
                        namespace,
                        dry_run=True,
                        field_manager="aiops-executor",
                    )
                    if not dry_ok:
                        return {
                            "success": False,
                            "error": f"Dry-Run 校验未通过: {name}",
                        }
    ...
    async def _execute_restart_step(
        self, step: Dict[str, Any], target: Dict[str, Any]
    ) -> Dict[str, Any]:
        """执行重启步骤"""
        try:
    ...
    async def _execute_monitor_step(
        self, step: Dict[str, Any], target: Dict[str, Any]
    ) -> Dict[str, Any]:
        """执行监控步骤"""
        try:
