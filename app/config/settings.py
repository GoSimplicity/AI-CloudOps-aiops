...
class LLMConfig:
    ...
    def effective_base_url(self) -> str:
        return (
            self.ollama_base_url if self.provider.lower() == "ollama" else self.base_url
        )
...
class K8sConfig:
    ...
    config_path: str = field(
        default_factory=lambda: get_env_or_config(
            "K8S_CONFIG_PATH", "kubernetes.config_path", "deploy/kubernetes/config"
        )
    )
    namespace: str = field(
        default_factory=lambda: get_env_or_config(
...
class RemediationConfig:
    ...
    enabled: bool = field(
        default_factory=lambda: get_env_or_config(
            "REMEDIATION_ENABLED", "remediation.enabled", True, bool
        )
    )
    dry_run: bool = field(
        default_factory=lambda: get_env_or_config(
            "REMEDIATION_DRY_RUN", "remediation.dry_run", True, bool
        )
    )
    allow_rollback: bool = field(
        default_factory=lambda: get_env_or_config(
            "REMEDIATION_ALLOW_ROLLBACK", "remediation.allow_rollback", True, bool
        )
    )
    verify_wait_seconds: int = field(
        default_factory=lambda: get_env_or_config(
            "REMEDIATION_VERIFY_WAIT", "remediation.verify_wait_seconds", 20, int
        )
    )
    max_concurrent: int = field(
        default_factory=lambda: get_env_or_config(
            "REMEDIATION_MAX_CONCURRENT", "remediation.max_concurrent", 3, int
        )
    )
    safe_mode: bool = field(
        default_factory=lambda: get_env_or_config(
            "REMEDIATION_SAFE_MODE", "remediation.safe_mode", False, bool
        )
    )
    namespace_whitelist: List[str] = field(
        default_factory=lambda: get_env_or_config(
            "REMEDIATION_NS_WHITELIST", "remediation.namespace_whitelist", [], list
        )
    )
    namespace_blacklist: List[str] = field(
        default_factory=lambda: get_env_or_config(
            "REMEDIATION_NS_BLACKLIST", "remediation.namespace_blacklist", [], list
        )
    )
...
class RCAConfig:
    ...
    request_override: bool = field(
        default_factory=lambda: get_env_or_config(
            "RCA_REQUEST_OVERRIDE", "rca.request_override", False, bool
        )
    )
...
class LogsConfig:
    ...
    enabled: bool = field(
        default_factory=lambda: get_env_or_config(
            "RCA_LOGS_ENABLED", "logs.enabled", False, bool
        )
    )
    tail_lines: int = field(
        default_factory=lambda: get_env_or_config(
            "RCA_LOGS_TAIL_LINES", "logs.tail_lines", 200, int
        )
    )
    max_pods: int = field(
        default_factory=lambda: get_env_or_config(
            "RCA_LOGS_MAX_PODS", "logs.max_pods", 5, int
        )
    )
    include_previous: bool = field(
        default_factory=lambda: get_env_or_config(
            "RCA_LOGS_INCLUDE_PREVIOUS", "logs.include_previous", False, bool
        )
    )
...
class TracingConfig:
    ...
    enabled: bool = field(
        default_factory=lambda: get_env_or_config(
            "RCA_TRACING_ENABLED", "tracing.enabled", False, bool
        )
    )
    provider: str = field(
        default_factory=lambda: get_env_or_config(
            "RCA_TRACING_PROVIDER", "tracing.provider", "jaeger"
        )
    )
    jaeger_query_url: str = field(
        default_factory=lambda: get_env_or_config(
            "RCA_JAEGER_QUERY_URL", "tracing.jaeger_query_url", "http://127.0.0.1:16686"
        )
    )
    timeout: int = field(
        default_factory=lambda: get_env_or_config(
            "RCA_TRACING_TIMEOUT", "tracing.timeout", 15, int
        )
    )
    service_name: Optional[str] = field(
        default_factory=lambda: get_env_or_config(
            "RCA_TRACING_SERVICE", "tracing.service_name", None
        )
    )
    max_traces: int = field(
        default_factory=lambda: get_env_or_config(
            "RCA_TRACING_MAX_TRACES", "tracing.max_traces", 30, int
        )
    )
...
class RedisConfig:
    ...
    decode_responses: bool = field(
        default_factory=lambda: get_env_or_config(
            "REDIS_DECODE_RESPONSES", "redis.decode_responses", False, bool
        )
    )
...
class DatabaseConfig:
    ...
    name: str = field(
        default_factory=lambda: get_env_or_config(
            "DB_NAME", "database.name", "cloudops"
        )
    )
    echo: bool = field(
        default_factory=lambda: get_env_or_config(
            "DB_ECHO", "database.echo", False, bool
        )
    )
    pool_size: int = field(
        default_factory=lambda: get_env_or_config(
            "DB_POOL_SIZE", "database.pool_size", 5, int
        )
    )
    max_overflow: int = field(
        default_factory=lambda: get_env_or_config(
            "DB_MAX_OVERFLOW", "database.max_overflow", 10, int
        )
    )
    pool_recycle: int = field(
        default_factory=lambda: get_env_or_config(
            "DB_POOL_RECYCLE", "database.pool_recycle", 1800, int
        )
    )
    ...
    def sqlalchemy_url(self) -> str:
        ...
        return f"mysql+pymysql://{user}:{pwd}@{self.host}:{self.port}/{self.name}?charset=utf8mb4"
...
class RAGConfig:
    ...
    retrieve_k: int = field(
        default_factory=lambda: get_env_or_config(
            "RAG_RETRIEVE_K", "rag.retrieve.k", 12, int
        )
    )
    score_threshold: float = field(
        default_factory=lambda: get_env_or_config(
            "RAG_SCORE_THRESHOLD", "rag.retrieve.score_threshold", 0.0, float
        )
    )
    hybrid_enabled: bool = field(
        default_factory=lambda: get_env_or_config(
            "RAG_HYBRID_ENABLED", "rag.retrieve.hybrid_enabled", False, bool
        )
    )
    reranker_enabled: bool = field(
        default_factory=lambda: get_env_or_config(
            "RAG_RERANKER_ENABLED", "rag.reranker.enabled", False, bool
        )
    )
    reranker_model: str = field(
        default_factory=lambda: get_env_or_config(
            "RAG_RERANKER_MODEL", "rag.reranker.model", "bge-reranker-base"
        )
    )
    reranker_top_k: int = field(
        default_factory=lambda: get_env_or_config(
            "RAG_RERANKER_TOP_K", "rag.reranker.top_k", 20, int
        )
    )
    iter_max_loops: int = field(
        default_factory=lambda: get_env_or_config(
            "RAG_ITER_MAX_LOOPS", "rag.iteration.max_loops", 1, int
        )
    )
    retry_confidence_threshold: float = field(
        default_factory=lambda: get_env_or_config(
            "RAG_RETRY_CONFIDENCE_THRESHOLD",
            "rag.iteration.retry_confidence_threshold",
            0.6,
            float,
        )
    )
    mmr_top_k: int = field(
        default_factory=lambda: get_env_or_config(
            "RAG_MMR_TOP_K", "rag.compression.mmr_top_k", 6, int
        )
    )
    mmr_lambda: float = field(
        default_factory=lambda: get_env_or_config(
            "RAG_MMR_LAMBDA", "rag.compression.mmr_lambda", 0.7, float
        )
    )
    answer_max_chars: int = field(
        default_factory=lambda: get_env_or_config(
            "RAG_ANSWER_MAX_CHARS", "rag.answer.max_chars", 300, int
        )
    )
    source_limit: int = field(
        default_factory=lambda: get_env_or_config(
            "RAG_SOURCE_LIMIT", "rag.answer.source_limit", 4, int
        )
    )
...
class MCPConfig:
    ...
    server_url: str = field(
        default_factory=lambda: get_env_or_config(
            "MCP_SERVER_URL", "mcp.server_url", "http://127.0.0.1:9000"
        )
    )
    timeout: int = field(
        default_factory=lambda: get_env_or_config(
            "MCP_TIMEOUT", "mcp.timeout", 120, int
        )
    )
