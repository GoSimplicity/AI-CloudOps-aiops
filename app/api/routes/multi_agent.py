...
@router.get("/metrics/detail", summary="获取多智能体指标详情", description="获取多智能体协作系统的运行指标，包括工作流统计、成功率等关键性能数据")
async def multi_agent_metrics():
    ...
    try:
        ...
        return APIResponse(code=0, message="多Agent指标获取成功", data=entity.model_dump()).model_dump()
...
@router.post("/repairs/create", summary="创建单个部署修复任务", description="为指定命名空间下的单个Kubernetes部署创建智能修复任务，通过多智能体协作进行故障诊断和自动修复")
...
@router.post("/repairs/create-all", summary="创建批量部署修复任务", description="为指定命名空间下的所有Kubernetes部署创建批量修复任务，通过多智能体协作批量处理故障修复")
...
@router.post("/analysis/create", summary="创建集群健康分析任务", description="对指定Kubernetes集群进行全面的健康状态分析，检测潜在问题并提供优化建议")
...
@router.get("/coordinator/status/detail", summary="获取协调器状态详情", description="获取多智能体协调器的详细运行状态，包括各组件健康状况和连接状态")
async def get_coordinator_status():
    ...
    try:
        ...
        entity = CoordinatorStatusEntity(healthy=bool(status.get("healthy")), components=status.get("components"))
        return APIResponse(code=0, message="协调器状态获取成功", data=entity.model_dump()).model_dump()
...
@router.get("/agents/list", summary="获取智能体列表", description="获取系统中所有可用智能体的列表信息，支持分页查询和关键词搜索")
async def list_agents(
    ...
    try:
        ...
        return PaginatedListAPIResponse(code=0, message="Agent列表获取成功", items=paginated_agents, total=total).model_dump()
...
@router.get("/health/detail", summary="多智能体服务健康检查", description="检查多智能体系统的整体健康状态，包括协调器和各个智能体组件的运行状况")
async def multi_agent_health():
    """多Agent服务健康检查"""
    try:
        # 检查协调器健康状态
        health = await coordinator.health_check()
        entity = CoordinatorStatusEntity(healthy=bool(health.get("healthy")), components=health.get("components"))
        data = entity.model_dump()
        data.update({"timestamp": iso_utc_now(), "service": "multi_agent"})
        return APIResponse(code=0, message="多Agent服务健康检查完成", data=data).model_dump()
...
@router.get("/status/detail", summary="获取多智能体系统状态", description="获取多智能体系统的当前运行状态和活跃智能体信息")
async def multi_agent_status():
    try:
        entity = MultiAgentStatusEntity(agents=[{"name": "detector", "status": "active"}])
...
@router.post("/execute", summary="执行多智能体任务", description="执行指定的多智能体协作任务，包括任务分发、执行监控和结果汇总")
...
@router.get("/coordination/detail", summary="获取智能体协调详情", description="获取多智能体协调机制的详细信息，包括任务分配、执行进度和资源利用率")
...
@router.get("/workflows/list", summary="获取工作流记录列表", description="获取多智能体工作流执行记录列表，支持按命名空间、状态筛选和分页查询")
    ...
    try:
        with session_scope() as session:
            ...
            rows = session.execute(
                stmt.order_by(WorkflowRecord.id.desc()).offset((page - 1) * size).limit(size)
            ).scalars().all()
            items = [
                WorkflowRecordEntity(
                    id=r.id,
...
@router.post("/workflows/create", summary="创建工作流记录", description="创建新的多智能体工作流执行记录，记录工作流的基本信息和执行状态")
...
@router.get("/workflows/detail/{record_id}", summary="获取工作流记录详情", description="根据记录ID获取指定工作流记录的详细信息，包括执行状态、目标资源等")
...
@router.put("/workflows/update/{record_id}", summary="更新工作流记录", description="更新指定ID的工作流记录信息，包括状态、目标资源和详细信息等")
...
@router.delete("/workflows/delete/{record_id}", summary="删除工作流记录", description="软删除指定ID的工作流记录，记录将被标记为已删除但不会从数据库中物理移除")
